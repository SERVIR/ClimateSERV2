...
def getSpatialReference(self, dataType):
    ...
    # print gridString
    geoTransform = (
    float(gridString[0]), float(gridString[1]), float(gridString[2]), float(gridString[3]), float(gridString[4]),
    float(gridString[5]))
    inputFileDef.close()
    ...
    def __openFileForWriting__(self, dataType, year):
        ...
        if (os.path.isfile(outputfile) != True):
            ...
            try:
                ...
                pass
            return self.f.create_dataset("data", (indexLastDay, size[1], size[0]), dtype='float32', compression="lzf",
