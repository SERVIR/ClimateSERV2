...
class ETL_Log(models.Model):
    ...
    etl_dataset = models.ForeignKey(ETL_Dataset, on_delete=models.SET_NULL, blank=True, null=True, help_text="If there is an associated ETL Dataset UUID, it should appear here.  Note: This field may be blank or unset, not all events will have one of these associated items.")
    etl_granule = models.ForeignKey(ETL_Granule, on_delete=models.SET_NULL, blank=True, null=True, help_text="If there is an associated ETL Granule UUID, it should appear here.  Note: This field may be blank or unset, not all events will have one of these associated items.")
