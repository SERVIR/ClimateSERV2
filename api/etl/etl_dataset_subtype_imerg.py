...
class ETL_Dataset_Subtype_IMERG(ETL_Dataset_Subtype, ETL_Dataset_Subtype_Interface):
    ...
    def __init__(self, etl_parent_pipeline_instance=None, dataset_subtype=None):
        ...
        if dataset_subtype == 'imerg_early_30min':
            self.mode = 'EARLY_30MIN'
        elif dataset_subtype == 'imerg_late_30min':
            self.mode = 'LATE_30MIN'
        elif dataset_subtype == 'imerg_early_1dy':
            self.mode = 'EARLY_1DY'
        elif dataset_subtype == 'imerg_late_1dy':
            self.mode = 'LATE_1DY'
        else:
            self.mode = 'LATE_1DY'
    ...
    def execute__Step__Pre_ETL_Custom(self):
        ...
        try:
            ...
            session.auth = (
                Config_SettingService.get_value(setting_name="FTP_CREDENTIAL_IMERG__USER", default_or_error_return_value="error_getting_user_name"),
                Config_SettingService.get_value(setting_name="FTP_CREDENTIAL_IMERG__PASS", default_or_error_return_value="error_getting_user_password")
            )
            ...
            for dt in rrule.rrule(rrule.MONTHLY, dtstart=start_date, until=end_date):
                ...
                for tif_filepath in tif_filepath_list:
                    ...
                    final_nc4_filename = 'nasa-imerg-{}.{}{}{}T{}Z.global.0.1deg.{}.nc4'.format(
                        'late' if self.mode.startswith('LATE') else 'early',
                        current_year,
                        current_month,
                        current_day,
                        '000000' if '1DY' in self.mode else current_time,
                        '1dy' if '1DY' in self.mode else '30min'
                    )
        ...
        for expected_granule in self._expected_granules:
            try:
                ...
                try:
                    ...
                    if not os.path.exists(os.path.dirname(local_full_filepath_tif)):
                        os.makedirs(os.path.dirname(local_full_filepath_tif))
                    ...
                    sysErrorData = str(sys.exc_info())
                    warn_JSON = {}
                    warn_JSON['warning'] = "Warning: There was an uncaught error when attempting to download file at URL: " + str(remote_full_filepath_tif) + ".  If the System Error message says something like 'nodename nor servname provided, or not known', then one common cause of that error is an unstable or disconnected internet connection.  Double check that the internet connection is working and try again.  System Error Message: " + str(sysErrorData)
    ...
    def execute__Step__Extract(self):
        ...
        ret__detail_state_info['class_name'] = self.__class__.__name__
        ret__detail_state_info['custom_message'] = "Imerg types do not need to be extracted.  The source files are non-compressed Tif files."
        ...
        try:
            ...
            for expected_granules_object in self._expected_granules:
                try:
                    ...
                    expected_full_path_to_local_extracted_tif_file = expected_granules_object['local_full_filepath_tif']
                    ...
                    hhmmss_start = 'S000000'
                    hhmmss_middle = 'S120000'
                    delta = pd.Timedelta(hours=23) + pd.Timedelta(minutes=59) + pd.Timedelta(seconds=59)
                    if '30MIN' in self.mode:
                        hhmmss_start = time_string[1]
                        hhmmss_middle = hhmmss_start
                        delta = pd.Timedelta(minutes=29) + pd.Timedelta(seconds=59)

                    # Determine the timestamp for the data.
                    start_time = datetime.datetime.strptime(yyyymmdd + hhmmss_start, '%Y%m%dS%H%M%S')
                    middle_time = datetime.datetime.strptime(yyyymmdd + hhmmss_middle, '%Y%m%dS%H%M%S')
                    ...
                    ds.attrs = OrderedDict([
                        ...
                        ('Source', 'NASA GPM Precipitation Processing System; {}'.format(self.current_root_http_path)),
        ...
        try:
            ...
            for expected_granules_object in self._expected_granules:
                ...
                try:
                    ...
                    print(expected_full_path_to_local_working_nc4_file)
